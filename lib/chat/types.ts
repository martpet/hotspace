...
export type ChatConnectionWithUser = Omit<ValidChatConnection, "userEntry"> & {
  userEntry: Deno.KvEntry<ChatUser>;
};

export type ChatEntryKvWatchCallback<T = ChatResource> = (arg: {
  entry: Deno.KvEntryMaybe<T>;
  prevEntry?: Deno.KvEntryMaybe<T>;
  chat: Chat;
}) => unknown;
...
export type GetKvQueueNonceFn = (
  nonce: string,
) => Promise<Deno.KvEntryMaybe<string>>;

export type DeleteKvQueueNonceFn = (nonce: string) => Promise<void>;

export type UsersKvKeyBuilder = (id: string) => Deno.KvKey;
