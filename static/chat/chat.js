...
function buildMsgEl(data) {
  ...
  mainEl.querySelector(".text").textContent = sanitizeChatMsgText(text);
  if (id) el.id = id;
...
function renderNetworkOnline() {
  if (networkOnlineSignal.get()) {
    document.getElementById("chat-offline")?.remove();
  } else {
    chatBox.insertAdjacentHTML(
      "afterbegin",
      "<p id='chat-offline' class='alert error'> Network is offline</p>",
    );
  }
}
